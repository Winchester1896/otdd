...
class DatasetDistance():
    ...
    def _init_data(self, D1, D2):
        ...
        if type(classes1[0]) == float:
            classes1 = [int(i) for i in classes1]
            # print('classes changed float to int ========')
        if type(targets1) == list:
            # print('target is a list =========')
            if type(targets1[0]) == float:
                # print('targets changed float to int ======')
                targets1 = [int(i) for i in targets1]
            targets1 = torch.Tensor(targets1).type(torch.int)
        # print(targets1, type(targets1), classes1, type(classes1), idxs1, type(idxs1))
        targets2, classes2, idxs2 = extract_data_targets(D2)
        if type(classes2[0]) == float:
            classes2 = [int(i) for i in classes2]
            # print('classes changed float to int ========')
        if type(targets2) == list:
            # print('target is a list =========')
            if type(targets2[0]) == float:
                # print('targets changed float to int ======')
                targets2 = [int(i) for i in targets2]
            targets2 = torch.Tensor(targets2).type(torch.int)
        # print(targets2, type(targets2), classes2, type(classes2), idxs2, type(idxs2))
        ...
        # print(self.classes1, self.classes2)
